...
export class RouteSheetBuilder {
  static async build(
    procedure: Procedure,
    workflow: workflow[],
    ...
    const docDefinition: TDocumentDefinitions = {
      ...
      content: [
        this.header(leftImage, rightImage),
        this.firstSection(procedure, workflow[0], isOriginal),
  ...
  private static header(leftImage: string, rightImage: string): Content {
    return {
      margin: [0, 0, 0, 5],
      columns: [
        {
  ...
  private static firstSection(
    ...
    const sentDetail = firstStep
      ? {
          ...
          ...this.formatDateTime(firstStep.sentDate),
        }
      : {
          officer: {
            fullname: '',
    ...
    const creationDetail = {
      ...
      ...this.formatDateTime(procedure.createdAt),
    };
  ...
  private static secondSection(workflow: workflow[]) {
    const sendeMap = this.buildSenderIndex(workflow);
    let containers: ContentTable[] = [];
    for (const [index, item] of workflow.entries()) {
      const nextStage = this.getNextStage(item, index, workflow, sendeMap);
      const container = this.stageContainer({
        ...
        receivedDate: {
          ...this.formatDateTime(nextStage?.sentDate),
    ...
    for (let index = length; index < this.getLastPageNumber(length); index++) {
      const container = this.stageContainer({
        index: index,
        officer: '',
        jobTitle: '',
  ...
  private static formatDateTime(date: string | Date | undefined) {
  ...
  private static getLastPageNumber(lengthData: number): number {
  ...
  private static buildSenderIndex(all: workflow[]): Map<string, workflow[]> {
