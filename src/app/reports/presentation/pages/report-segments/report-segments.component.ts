...
@Component({
  ...
  template: `
    <div class="container mx-auto p-4">
      ...
      <form [formGroup]="filterForm" (ngSubmit)="getData()" class="mb-6 p-4">
      ...
      @if(!isLoading() && (results()?.segments?.length ?? 0) > 0){
        <div class="mb-8">
          <h2 class="text-xl font-semibold mb-4">Totales Registros</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="flex items-center p-4">
              <span class="text-xl font-medium mr-2">Pendientes:</span>
              <span class="text-2xl font-bold text-orange-500">
                {{ results()?.globalTotals?.pending }}
              </span>
            </div>
            <div class="flex items-center ">
              <span class="text-xl font-medium mr-2">Completados:</span>
        ...
        <h2 class="text-xl font-semibold mb-6">Detalle por Segmento</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <ng-container *ngIf="results()?.segments; else noSegments">
